...
from .utils.headers import (
    ...
    group,
    get_raw_tag_value,
    force_change_header_value,
)
from .utils.io import load_dicom, save_omids, load_dicom_with_subfolders, load_scanco
import pathlib
...
def convert_scanco_to_ormirmids(
    input_folder,
    output_folder,
    anonymize="anon",
    recursive=True,
    session="",
    series_number=False,
    save_patient_json=True,
    save_extra_json=True,
):
    """
    Convert Scanco to ORMIR-MIDS format.

    Parameters:
    - input_folder (str): Path to the input folder with Scanco files.
    ...
    # ! First issue: load_dicom are unaware of the file format of the files within inputDir. How do we check if DICOM or AIM/ISQ/etc?
    # if RECURSIVE:
    #     med_volume_list = load_dicom_with_subfolders(inputDir)
    # else:
    #     med_volume_list = load_dicom(inputDir)

    med_volume_list = load_scanco(inputDir)

    print("Data loaded")
    
    # ! Second issue: AIM/ISQ files do not have the same header structure as DICOM files, so they need to be handled differently;
    # ! or assume a basic set of dicom tags and write a fake dicom header for the MedicalVolume object.
    # TODO: continue from here (POS, 01.07.2026)
        ...
        for series_group_name, series_list in multiseries_config.items():
            if not isinstance(series_list, list):
                try:
                    ...
                    print(
                        "Error parsing multiseries config for group", series_group_name
                    )
                    continue
                new_multiseries_config[series_group_name] = series_list
                print(
                    "Multiseries config for group", series_group_name, ":", series_list
                )
            else:
                new_multiseries_config[series_group_name] = series_list
                ...
                if series_number.startswith("["):
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(
                                "Error parsing series number in overrides, not an integer, list expression, or multiseries config reference:",
                                series_number,
                            )
                            continue
            else:
    ...
    for med_volume in med_volume_list:
        ...
        if series_number in overrides:
            for key, value in overrides[series_number].items():
                print(
                    "Applying override for series", series_number, ":", key, "=", value
                )
    ...
    def convert_recursive(converter_class, med_volume):
        ...
        if multiseries_part == converter_class.is_multiseries():
            ...
            if ANON_NAME:
                ...
                patient_name = parse_patient_name(
                    med_volume.patient_header["PatientName"]
                )
            output_path = pathlib.Path(outputDir) / os.path.dirname(
                converter_class.get_file_path(patient_name, session)
            )
            output_path.mkdir(parents=True, exist_ok=True)
            if multiseries_part:
                if multiseries_finished is not None:
                    # a multiseries is finished, we can concatenate
                    concat_volume_4d = concatenate_volumes_3d(
                        multiseries_volumes[series_group_name]
                    )
                    converted_multiseries_volume = group(
                        concat_volume_4d, converter_class.multiseries_concat_tag()
                    )
                    ...
                    if ADD_SERIES_NUMBER:
                        first_series = min(
                            [
                                get_raw_tag_value(x, "00200011")[0]
                                for x in multiseries_volumes[series_group_name]
                            ]
                        )
                    ...
                    save_omids(
                        str(
                            ...
                            / (
                                series_prefix
                                + converter_class.get_file_name(patient_name)
                            )
                        )
                        + ".nii.gz",
                    ...
                    print(
                        "Volume",
                        med_volume.path,
                        converted_multiseries_volume.shape,
                        "saved with",
                        converter_class.get_name(),
                        "using multiseries concatenation",
                    )
            ...
            save_omids(
                str(
                    output_path
                    / (series_prefix + converter_class.get_file_name(patient_name))
                )
                + ".nii.gz",
                converted_volume,
                save_patient_json,
    ...
    for med_volume in med_volume_list:
        ...
        if multiseries_config:
            ...
            for series_group_name, series_list in multiseries_config.items():
                ...
                if series_number in series_list or med_path in [
                    os.path.abspath(os.path.join(inputDir, str(x))) for x in series_list
                ]:
...
def main():
    ...
    input_folder = os.path.abspath(
        r"C:\Users\ADMIN\Documents\02_PROJECTS\ORMIR\test_compatibility\raw_data\test_header"
    )
    output_folder = os.path.abspath(
        r"C:\Users\ADMIN\Documents\02_PROJECTS\ORMIR\test_compatibility\output"
    )
    anonymize = "anon"
    recursive = False
    series_number = False
    ...
    convert_scanco_to_ormirmids(
        ...
        SESSION,
        ADD_SERIES_NUMBER,
        # not args.disable_patient_json,
        # not args.disable_extra_json,
        False,
        False,
    )
...
if __name__ == "__main__":
    # * Prototyping for compatibility with SCANCO aim and isq images
    main()
